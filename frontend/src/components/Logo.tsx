...
const Logo: React.FC<LogoProps> = ({ size = 24, className = "", grayscale = false }) => {
  return (
    <div 
      className={`relative flex-shrink-0 flex items-center justify-center ${className}`} 
      style={{ width: size, height: size }}
    >
      {/* Dynamic Shining/Glow Background Background - Hidden in grayscale mode */}
      {!grayscale && (
      ...
      {/* The Logo Image */}
      <div className={`relative z-10 w-full h-full rounded-full overflow-hidden border border-white/20 shadow-lg ${grayscale ? 'grayscale opacity-60' : ''}`}>
        <img 
          src="/logo.jpg" 
          alt="DriveSafe Logo" 
